...
server/views/layouts/portal.ejs       # EJS shell served by Express
  └── div.portal                      # Mount target for React SPA
...
3. The EJS shell includes the compiled bundle and exposes a `.portal` div.
4. `src/react/app/mount-portal.tsx` calls `mountPortalApp()`, which mounts `PortalApp.tsx` into `.portal`.
...
All routes are hash-based (e.g., `/#/dashboard`). `PortalRoutes.tsx` branches on `user.role` to render either the admin or client component for shared paths. The table below reflects what clients see.

### Client routes (rendered components)
...
| `/documents` | `PortalDocuments` | Consolidated documents view (invoices, contracts, agreements) |
| `/files` | `PortalFilesHub` | File management hub |
...
| `/proposals/:id` | `PortalProposalDetail` | Proposal detail view with accept/decline |
| `/agreements` | `AgreementsList` | Project agreements list |
| `/agreements/:id` | `AgreementFlow` | Step-by-step agreement flow |
| `/meetings` | `MeetingRequestsList` | Meeting request submission and tracking |
| `/retainers` | `PortalRetainers` | Active retainer agreements with utilization |
| `/feedback` | `PortalFeedback` | Pending and completed feedback surveys |
| `/payment-schedule` | `PaymentScheduleView` | Payment installment tracking |
| `/auto-pay` | `AutoPaySettings` | Auto-pay configuration |
| `/requests-hub` | `PortalRequestsHub` | Unified hub: ad hoc requests, questionnaires, document requests |
| `/content-requests` | `ContentChecklistView` | Content request checklists |
| `/help` | `PortalHelp` | Help center |

### Client redirects (legacy paths)

These paths are accepted but redirect clients to the consolidated routes above:

| Hash Route | Redirects To | Reason |
|---|---|---|
| `/invoices` | `/documents` | Invoices consolidated into Documents view |
| `/projects` | `/dashboard` | Project list/detail folded into dashboard |
| `/questionnaires` | `/requests-hub` | Consolidated into Requests Hub |
| `/document-requests` | `/requests-hub` | Consolidated into Requests Hub |
| `/requests` | `/requests-hub` | Consolidated into Requests Hub |
| `/approvals` | `/deliverables` | Approvals folded into Deliverables Hub |
| `/review` | `/dashboard` | Legacy path |

Shared routes (`/dashboard`, `/messages`, `/files`, `/settings`, `/contracts`, `/deliverables`, `/proposals`, `/agreements`, `/meetings`, `/retainers`, `/feedback`, `/documents`) render different components based on `user.role`.
...
Displays an overview of the client's account: quick stats (active projects, pending invoices, unread messages), project cards with progress, and a recent activity feed. The client project list and detail views were folded into the dashboard — `/projects` redirects here.
...
Consolidated view of invoices, contracts, and agreements. `/invoices` redirects here.

### Files Hub

**Directory:** `files-hub/`

**Component:** `PortalFilesHub`

File management hub. Wraps the underlying file manager (`files/`) with the client-facing layout.
...
**Directory:** `deliverables-hub/`

**Component:** `PortalDeliverablesHub`

Deliverable cards with approval flow. `/approvals` redirects here.

### Proposals

**Directory:** `proposals/`

**Components:** `PortalProposals`, `PortalProposalDetail`, `ProposalCard`

View and respond to proposals. Detail view (`/proposals/:id`) supports accept/decline.

### Agreements

**Directory:** `agreements/`

**Components:** `AgreementsList`, `AgreementFlow`

Unified flow combining proposal review, contract signing, deposit payment, and questionnaire into one step-by-step experience.
...
Meeting request submission and tracking.

### Retainers

**Directory:** `retainers/`

**Component:** `PortalRetainers`

Active retainer agreements with utilization tracking.
...
Pending and completed feedback surveys.

### Payment Schedule

**Directory:** `payment-schedule/`
...
Auto-pay configuration for recurring invoices.

### Requests Hub

**Directory:** `requests-hub/`

**Component:** `PortalRequestsHub`

Unified hub for ad hoc requests, questionnaires, and document requests. The legacy `/requests`, `/questionnaires`, and `/document-requests` routes redirect here.

### Content Requests

**Directory:** `content-requests/`

**Component:** `ContentChecklistView`

Content request checklists.
...
**Components:** `StripePaymentForm`, `StripeProvider`

Stripe Elements integration. Embedded in agreement deposit step and standalone invoice payment.
...
### Legacy / Non-routed Directories

The following directories still exist under `src/react/features/portal/` but no longer have client routes — their functionality was consolidated. They may export shared types or components used by the hub views:

- `invoices/` — folded into `documents/`
- `projects/` — folded into `dashboard/`
- `questionnaires/` — folded into `requests-hub/`
- `ad-hoc-requests/` — folded into `requests-hub/`
- `document-requests/` — folded into `requests-hub/`
- `approvals/` — folded into `deliverables-hub/`
- `deliverables/` — superseded by `deliverables-hub/`
- `files/` — wrapped by `files-hub/`
- `preview/` — no active route
...
For deeper detail on individual features, see the dedicated docs below.
...
### 2026-04-30 - Routing and components accuracy pass

- Replaced routing table to reflect role-based branching in `PortalRoutes.tsx` — split into "Client routes" (rendered components) and "Client redirects" (legacy paths). Documented that most paths render different components for admin vs client.
- Added missing client routes: `/documents` (`PortalDocuments`), `/auto-pay` (`AutoPaySettings`).
- Documented redirects: `/invoices` → `/documents`, `/projects` → `/dashboard`, `/questionnaires` → `/requests-hub`, `/document-requests` → `/requests-hub`, `/requests` → `/requests-hub`, `/approvals` → `/deliverables`, `/review` → `/dashboard`.
- Replaced Feature Components section to match the consolidated hub architecture: Documents, Files Hub, Deliverables Hub, Requests Hub, Content Requests, Payment Schedule, Auto-Pay, Onboarding Checklist, Payments. Removed entries for components no longer routed for clients (`PortalInvoicesTable`, `PortalProjectsList`, `PortalProjectDetail`, `PortalQuestionnairesView`, `PortalAdHocRequests`, `PortalApprovals`, `PortalDocumentRequests`, `PortalPreview`).
- Added "Legacy / Non-routed Directories" subsection listing portal feature dirs that still exist on disk but are no longer routed.
- Renamed "Roadmap Features (Phase 1-5A)" to "Feature Deep-Dives" and dropped Phase labels (features are shipped).
