...
> **⚠️ Outdated / legacy (as of 2026-06-25):** This document describes the retired vanilla-TypeScript portal era. It was written when the client portal still ran on vanilla TypeScript modules (`portal-navigation.ts`, `client-portal.ts`, `portal-views.ts`, etc.) and references components (e.g. `breadcrumbs.ts`, `tab-router.ts`, `quick-stats.ts`, `recent-activity.ts`, `page-header.ts`, `search-bar.ts`, `page-title.ts`) and pages (`admin/index.html`) that no longer exist. Those files have since been removed and the client portal is now a React SPA sharing the same component tree as the admin; the StatusBadge / IconButton / DataTable gaps flagged below have since been built as React components. **For the current state, see [COMPONENT_LIBRARY.md](./COMPONENT_LIBRARY.md) (component catalog) and [FACTORY_SYSTEM.md](./FACTORY_SYSTEM.md) (factory system).** For current portal architecture see [PORTAL_ARCHITECTURE.md](../features/PORTAL_ARCHITECTURE.md) and [CLIENT_PORTAL.md](../features/CLIENT_PORTAL.md). Wireframe mode (`?wireframe=1`) remains valid in spirit; specific file paths and the reusable-component gap analysis below are historical.
