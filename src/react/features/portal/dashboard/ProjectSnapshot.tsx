 ...
 * - ProjectHeader: project name, status badge, preview link (uses DetailHeader)
 * - ProjectProgress: progress bar, current deliverable (for panel wrapping)
 */
...
import type { MetaField } from '@react/components/portal/DetailHeader';
...
/** Page-level project context: name + status + preview link. */
export const ProjectHeader = React.memo(({ project }: ProjectHeaderProps) => {
  ...
  if (project.startDate || project.endDate) {
    meta.push({
      label: 'Timeline',
      value: `${project.startDate ? formatCardDate(project.startDate) : 'TBD'} \u2192 ${project.endDate ? formatCardDate(project.endDate) : 'TBD'}`,
      icon: <Calendar className="icon-xs" />
    });
  }
...
/** Progress bar, current deliverable. Intended to be wrapped in .panel. */
