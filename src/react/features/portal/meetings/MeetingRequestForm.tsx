...
import { MEETING_TYPE_LABELS, MEETING_DURATIONS, DEFAULT_DURATION_MINUTES } from './types';
...
export const MeetingRequestForm = React.memo(
  ({ projectId, onSubmit, onCancel, getAuthToken, showNotification }: MeetingRequestFormProps) => {
    ...
    const handleSubmit = async (e: React.FormEvent) => {
      ...
      try {
        ...
        if (preferredSlot2) {
          body.preferredSlot2 = preferredSlot2;
        }
        if (preferredSlot3) {
          body.preferredSlot3 = preferredSlot3;
        }
        if (projectId) {
        ...
        showNotification?.(formatErrorMessage(err, 'Failed to submit meeting request'), 'error');
      } finally {
    ...
    return (
      <form onSubmit={handleSubmit}>
        ...
        <div className="flex flex-col gap-1">
          ...
          {errors.meetingType && <span className="form-error-message">{errors.meetingType}</span>}
        </div>

        {/* Preferred Slot 1 (required) */}
        <div className="flex flex-col gap-1 mt-3">
          <label className="field-label" htmlFor="preferred-slot-1">
            Preferred Time Slot 1<span className="form-required">*</span>
          </label>
          <input
            id="preferred-slot-1"
            type="datetime-local"
        ...
        <div className="flex flex-col gap-1 mt-3">
          <label className="field-label" htmlFor="preferred-slot-2">
            Preferred Time Slot 2<span className="text-muted"> (optional)</span>
          </label>
          <input
            id="preferred-slot-2"
            type="datetime-local"
        ...
        <div className="flex flex-col gap-1 mt-3">
          <label className="field-label" htmlFor="preferred-slot-3">
            Preferred Time Slot 3<span className="text-muted"> (optional)</span>
          </label>
          <input
            id="preferred-slot-3"
            type="datetime-local"
