...
import { ListTodo, CheckCircle, Clock, Box, ArrowRight } from 'lucide-react';
  ...
  return (
    <div className="panel">
      <div className="data-table-header">
        <h3>
          <span className="title-full">Milestones</span>
        </h3>
        {onNavigateToDeliverables && (
          <div className="data-table-actions">
            <button className="panel-action" onClick={onNavigateToDeliverables}>
      ...
      {milestones.length === 0 ? (
        <EmptyState icon={<ListTodo className="icon-lg" />} message="No milestones yet." />
      ) : (
        <div className="milestone-list">
          {milestones.map((milestone) => {
            const milestoneTasks = tasks.filter((t) => t.milestone_id === milestone.id);
            const completedTaskCount = milestoneTasks.filter(
              (t) => t.status === 'completed'
            ).length;
            const allTasksDone =
              milestoneTasks.length > 0 && completedTaskCount === milestoneTasks.length;
