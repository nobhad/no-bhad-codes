  ...
  const handleGenerateContract = async () => {
    try {
      await downloadFromUrl(
        buildEndpoint.contractPdf(project.id),
        `contract-${project.project_name || project.id}.pdf`
      );
  ...
  const handleSendForSignature = async () => {
    try {
      const res = await apiPost(
        `${buildEndpoint.contractPdf(project.id).replace('/pdf', '/request-signature')}`,
        {}
      );
      if (res.ok) {
    ...
    <div className="subsection">
        ...
        <div className="contract-status-layout">
          <div>
            <div className="layout-row">
              {isSigned ? <Check className="icon-md" /> : <FileSignature className="icon-md" />}
              <h3 className="heading">
                <span className="title-full">Contract Status</span>
              </h3>
              <span className="badge">{contractStatus.label}</span>
            </div>
          ...
          <div className="detail-actions">
            ...
            {isSigned && onUpdateProject && (
              <IconButton
                action="restore"
                onClick={handleClearSignedDate}
                title="Reset signed status"
              />
            )}
            <IconButton
              action="generate"
            ...
            {contractFiles.map((file) => (
              <div key={file.id} className="list-item">
                <div className="layout-row">
                  ...
                  <div>
                    <span className="pd-highlight-value">{file.original_name}</span>
      ...
      {!isSigned && project.status === 'active' && (
        <div className="portal-card contract-warning">
          ...
          <div>
            <h4 className="heading">Contract Not Signed</h4>
            <p className="text-secondary pd-mt-1">
              This project is active but the contract has not been signed. Consider sending the
              contract for signature before proceeding with work.
            </p>
          </div>
        </div>
      )}
