...
interface PortfolioProject {
  ...
  testUrl?: string;
  repoUrl?: string;
  // Long-form documentation for the project — e.g. a generated design
  // system reference. `docsLabel` names the link; defaults to
  // "Documentation" so the field stays reusable across projects.
  docsUrl?: string;
  docsLabel?: string;
  launchDate?: string;
...
// Documentation icon (design system, spec pages)
const DOCS_SVG = `
<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
...
export class ProjectsModule extends BaseModule {
  ...
  private async playTuneInSequence(slug: string): Promise<void> {
    ...
    void tvSfx.static({
      ...
      //
      // 0.12 -> 0.05: this burst lands on top of the channel's music, and
      // broadband noise reads much louder than a tonal track at the same
      // amplitude. The power-on crackle keeps its own level — nothing is
      // playing under that one.
      peakGain: 0.05,
      attackS: 0.02,
      holdS: 0.07,
      dropToFraction: 0.35,
