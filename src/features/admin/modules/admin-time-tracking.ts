...
function renderEntryRow(entry: TimeEntry): string {
  return `
    <tr>
      ...
      <td>
        <button class="btn btn-outline btn-sm btn-edit-entry" data-entry-id="${entry.id}">Edit</button>
        <button class="btn btn-danger btn-sm btn-delete-entry" data-entry-id="${entry.id}">Delete</button>
      </td>
    </tr>
  `;
}
