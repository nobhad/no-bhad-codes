 ...
 * ===============================================
 * @file src/features/admin/modules/admin-messages.ts
 *
...
import { createMessageAttachmentManager, AttachmentManager } from '../../../utils/attachment-manager';
...
const attachmentManager: AttachmentManager = createMessageAttachmentManager({
  onError: (msg) => showToast(msg, 'error')
});

/**
 * Encapsulated module state to prevent scattered mutable globals
 */
interface MessagingModuleState {
...
const moduleState: MessagingModuleState = {
...
/** Reset module state (for cleanup/testing) */
export function resetModuleState(): void {
  moduleState.selectedClientId = null;
  moduleState.selectedThreadId = null;
  moduleState.selectedClientName = 'Client';
  moduleState.listenersInitialized = false;
}
...
export function getSelectedThreadId(): number | null {
  return moduleState.selectedThreadId;
}

export function getSelectedClientId(): number | null {
  return moduleState.selectedClientId;
}
...
export async function loadClientThreads(ctx: AdminDashboardContext): Promise<void> {
  ...
  try {
    ...
    // Debug: log clients with their threads
    logger.log('Loaded clients with threads:', clientsWithThreads.map(c => ({
...
function renderThreadList(
  ...
  container.innerHTML = clients
    .map((client) => {
      const isActive = client.client_id === moduleState.selectedClientId;
  ...
  if (mobileSelect) {
    const options = clients
      .map((client) => {
        ...
        const isSelected = client.client_id === moduleState.selectedClientId;
...
function setupThreadListHandlers(container: HTMLElement, ctx: AdminDashboardContext): void {
  container.querySelectorAll('.thread-item').forEach((item) => {
    const handleSelect = async () => {
      ...
      moduleState.selectedClientName = clientName;
...
export function selectThread(clientId: number, threadId: number, ctx: AdminDashboardContext): void {
  logger.log('selectThread called:', { clientId, threadId });
  moduleState.selectedClientId = clientId;
  moduleState.selectedThreadId = threadId;
...
export async function loadThreadMessages(
  ...
  try {
    ...
    if (response.ok) {
      const data = await parseApiResponse<{ thread: unknown; messages: Message[] }>(response);
      logger.log('loadThreadMessages response:', { threadId, messageCount: (data.messages || []).length, messages: data.messages });
      renderMessages(data.messages || [], container);

      // Mark messages as read
      await apiPut(`/api/messages/threads/${threadId}/read`);
    } else {
      logger.error('loadThreadMessages failed:', { threadId, status: response.status });
    ...
    .map(
      (
        ...
        const rawSenderName = isAdmin
          ? 'You'
          : SanitizationUtils.decodeHtmlEntities(moduleState.selectedClientName || 'Client');
...
function setupMessageInteractions(container: HTMLElement): void {
  ...
  container.querySelectorAll('.pin-message-btn').forEach((btn) => {
    btn.addEventListener('click', async (e) => {
      ...
      if (messageId && moduleState.selectedThreadId) {
...
async function addReaction(messageId: number, reaction: string): Promise<void> {
  try {
    ...
    if (moduleState.selectedThreadId) {
      const container = getMessagesContainer();
      if (container) {
        await loadThreadMessagesWithReactions(moduleState.selectedThreadId, container);
      }
    }
  } catch (error) {
...
async function togglePin(messageId: number, isPinned: boolean): Promise<void> {
  if (!moduleState.selectedThreadId) return;
  try {
    if (isPinned) {
      // Unpin: DELETE with thread_id as query param
      await apiDelete(`/api/messages/${messageId}/pin?thread_id=${moduleState.selectedThreadId}`);
    } else {
      // Pin: POST with thread_id in body
      await apiPost(`/api/messages/${messageId}/pin`, { thread_id: moduleState.selectedThreadId });
    }
    // Reload messages
    const container = getMessagesContainer();
    if (container) {
...
export async function sendMessage(ctx: AdminDashboardContext): Promise<void> {
  ...
  if (!input || !moduleState.selectedThreadId) return;
  ...
  try {
    ...
    if (hasAttachments) {
      ...
      response = await apiFetch(`/api/messages/threads/${moduleState.selectedThreadId}/messages`, {
        method: 'POST',
        body: formData
      });
    } else {
    ...
    if (response.ok) {
      ...
      loadThreadMessages(moduleState.selectedThreadId, ctx, true);
...
export function setupMessageListeners(ctx: AdminDashboardContext): void {
  // Guard against duplicate initialization
  if (moduleState.listenersInitialized) {
    logger.debug('Messaging listeners already initialized, skipping');
    return;
  }
  ...
  moduleState.listenersInitialized = true;
