 ...
 * ===============================================
 * @file server/utils/access-control.ts
 *
 * Centralized access control helpers for verifying
 * user permissions to access various resources.
 *
 * NOTE: These are utility functions, not Express middleware.
 * They are used by routes to check permissions.
 */
 ...
 */
export async function isUserAdmin(req: JWTAuthRequest): Promise<boolean> {
 ...
 */
export async function canAccessProject(req: JWTAuthRequest, projectId: number): Promise<boolean> {
 ...
 */
export async function canAccessInvoice(req: JWTAuthRequest, invoiceId: number): Promise<boolean> {
  if (await isUserAdmin(req)) {
    return true;
  }
 ...
 */
export async function canAccessFolder(req: JWTAuthRequest, folderId: number): Promise<boolean> {
  if (await isUserAdmin(req)) {
    return true;
  }
 ...
 */
export async function canAccessTask(req: JWTAuthRequest, taskId: number): Promise<boolean> {
  if (await isUserAdmin(req)) {
    return true;
  }
 ...
 */
export async function canAccessThread(req: JWTAuthRequest, threadId: number): Promise<boolean> {
  if (await isUserAdmin(req)) {
    return true;
  }
