 ...
 * ===============================================
 * @file server/services/project-generator.js
 * 
 ...
 */
export async function generateProjectPlan(intakeData, projectId) {
...
function calculateFeatureAdditions(features, projectType) {
...
function calculateAddonAdditions(addons) {
  const addonPricing = {
...
function getComplexityMultiplier(intakeData) {
  let multiplier = 1.0;
...
function generateDetailedPhases(phases, intakeData, features, addons) {
  const phaseDetails = {
    'Planning & Design': {
      duration: '3-5 days',
...
function generateTechnicalRequirements(intakeData) {
  const requirements = [];
...
function generateAssumptions(intakeData) {
  const assumptions = [
...
function generateRisks(intakeData) {
  const risks = [
...
function generatePaymentSchedule(totalPrice, estimatedWeeks) {
  if (totalPrice < 3000) {
    return [
