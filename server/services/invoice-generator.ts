...
interface LineItem {
  id?: number;
  description: string;
  type: 'base' | 'feature' | 'addon' | 'adjustment';
  quantity: number;
  unitPrice: number;
  totalPrice: number;
...
function generateAddonLineItems(addons: string[]): LineItem[] {
...
function calculateComplexityAdjustments(intakeData: IntakeData): LineItem[] {
...
function generatePaymentTerms(total: number, timeline: string): PaymentTerm[] {
...
function generateInvoiceNumber(): string {
  ...
  const random = (crypto.randomBytes(2).readUInt16BE(0) % 1000).toString().padStart(3, '0');

  return `INV-${year}${month}${day}-${random}`;
}

function generateInvoiceNotes(intakeData: IntakeData): string[] {
  const notes = [
