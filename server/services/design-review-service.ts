...
class DesignReviewService {
  ...
  async getAll(projectId?: string | number): Promise<DesignReviewListItem[]> {
    ...
    let whereClause =
      "WHERE d.status IN ('ready_for_review', 'revision_requested', 'approved') AND d.deleted_at IS NULL AND p.deleted_at IS NULL AND c.deleted_at IS NULL";
    ...
    `,
      params
    ) as Promise<DesignReviewListItem[]>;
  }
  ...
  async getById(reviewId: number): Promise<DesignReviewDetail | undefined> {
    ...
    `,
      [reviewId]
    ) as Promise<DesignReviewDetail | undefined>;
  }
  ...
  async getAttachments(reviewId: number): Promise<DesignReviewAttachment[]> {
    ...
    `,
      [reviewId]
    ) as Promise<DesignReviewAttachment[]>;
  }
  ...
  async create(data: {
    ...
    return db.get(`SELECT ${DELIVERABLE_COLUMNS} FROM deliverables WHERE id = ?`, [
      result.lastID
