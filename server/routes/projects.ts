...
router.post('/', authenticateToken, requireAdmin, asyncHandler(async (req: express.Request, res: express.Response) => {
  const {
    client_id,
    name,
    description,
...
router.put('/:id', 
  ...
  if (req.user!.type === 'admin') {
    ...
    project = await db.get('SELECT * FROM projects WHERE id = ? AND client_id = ?', [projectId, req.user!.id]);
  }
  ...
  const values: any[] = [];
  const allowedUpdates = req.user!.type === 'admin' 
    ...
    try {
      ...
      if (client) {
        const statusDescriptions: { [key: string]: string } = {
          ...
          'on-hold': 'Your project has been temporarily paused. We\'ll keep you updated on next steps.'
        };
        ...
        await emailService.sendProjectUpdateEmail(client.email, {
          ...
          nextSteps: req.body.status === 'completed' 
...
router.post('/:id/files', authenticateToken, upload.array('files', 5), asyncHandler(async (req: AuthenticatedRequest, res: express.Response) => {
  const projectId = parseInt(req.params.id);
  ...
  if (req.user!.type === 'admin') {
    ...
    project = await db.get('SELECT id FROM projects WHERE id = ? AND client_id = ?', [projectId, req.user!.id]);
  }
...
router.post('/:id/messages', authenticateToken, asyncHandler(async (req: AuthenticatedRequest, res: express.Response) => {
  const projectId = parseInt(req.params.id);
...
router.put('/:id/messages/read', authenticateToken, asyncHandler(async (req: AuthenticatedRequest, res: express.Response) => {
  const projectId = parseInt(req.params.id);
  const db = getDatabase();
...
router.get('/:id/milestones', authenticateToken, asyncHandler(async (req: AuthenticatedRequest, res: express.Response) => {
  const projectId = parseInt(req.params.id);
  const db = getDatabase();
...
router.post('/:id/milestones', authenticateToken, requireAdmin, asyncHandler(async (req: express.Request, res: express.Response) => {
  const projectId = parseInt(req.params.id);
...
router.post('/:id/updates', authenticateToken, requireAdmin, asyncHandler(async (req: express.Request, res: express.Response) => {
  const projectId = parseInt(req.params.id);
...
router.get('/:id/dashboard', authenticateToken, asyncHandler(async (req: AuthenticatedRequest, res: express.Response) => {
  const projectId = parseInt(req.params.id);
  const db = getDatabase();
  ...
  const progressPercentage = stats && stats.total_milestones > 0 
