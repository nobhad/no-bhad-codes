 ...
 */
router.get('/stream', authenticateToken, (req: AuthenticatedRequest, res: express.Response) => {
  const user = req.user!;
  ...
  const clientId = sseManager.addClient(res, user.id, user.email, user.type as 'admin' | 'client');
 ...
 */
router.post('/typing', authenticateToken, (req: AuthenticatedRequest, res: express.Response) => {
  const user = req.user!;
